...
class WDisplayArea internal constructor(private val displayArea: ComPtr) {
    ...
    val isPrimary: Boolean get() = displayArea.getBool(Abi.IDisplayArea_get_IsPrimary)
    ...
    val workArea: WRectangle
        get() = XamlStructs.getRectInt32(displayArea, Abi.IDisplayArea_get_WorkArea).let {
            WRectangle(it[0], it[1], it[2], it[3])
        }

    /** The display's whole area (DisplayArea.OuterBounds). */
    val outerBounds: WRectangle
        get() = XamlStructs.getRectInt32(displayArea, Abi.IDisplayArea_get_OuterBounds).let {
            WRectangle(it[0], it[1], it[2], it[3])
        }

    companion object {
        private val statics: ComPtr by lazy {
            Activation.factory(Abi.CLS_DisplayArea, Abi.IID_IDisplayAreaStatics)
        }

        /** Gets the primary display's DisplayArea (DisplayAreaStatics.Primary). */
        fun primary(): WDisplayArea = WDisplayArea(statics.getPtr(Abi.IDisplayAreaStatics_get_Primary))
        ...
        fun nearest(appWindow: WAppWindow): WDisplayArea = Ffi.backend.withScope { scope ->
            ...
            WDisplayArea(
                statics.getPtr(
                    Abi.IDisplayAreaStatics_GetFromWindowId, windowId, Abi.DisplayAreaFallback_Nearest,
                ),
            )
        }
    }
