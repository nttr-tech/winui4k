...
class WCalendarDatePicker : WControl(
    Activation.composeDefault(Abi.CLS_CalendarDatePicker, Abi.IID_ICalendarDatePickerFactory),
) {
    ...
    var date: LocalDate?
        get() {
            val boxed = inspectable.getPtrOrNull(Abi.ICalendarDatePicker_get_Date) ?: return null
        ...
        set(value) {
            if (value == null) {
                inspectable.call(Abi.ICalendarDatePicker_put_Date, null)
            } else {
                val boxed = PropertyValues.boxDateTime(DateTimeConversions.localDateToTicks(value))
                inspectable.call(Abi.ICalendarDatePicker_put_Date, boxed.ptr)
                boxed.release()
            }
        }
    ...
    var isCalendarOpen: Boolean
        get() = inspectable.getBool(Abi.ICalendarDatePicker_get_IsCalendarOpen)
        set(value) = inspectable.putBool(Abi.ICalendarDatePicker_put_IsCalendarOpen, value)

    /** The text shown when no date is selected (CalendarDatePicker.PlaceholderText). */
    var placeholderText: String
        get() = inspectable.getString(Abi.ICalendarDatePicker_get_PlaceholderText)
        set(value) = Hstring.use(value) { inspectable.call(Abi.ICalendarDatePicker_put_PlaceholderText, it) }

    /** The heading text (CalendarDatePicker.Header). */
    var header: String
        get() {
            val boxed = inspectable.getPtrOrNull(Abi.ICalendarDatePicker_get_Header) ?: return ""
        ...
        set(value) {
            ...
            inspectable.call(Abi.ICalendarDatePicker_put_Header, boxed.ptr)
            boxed.release()
        }
    ...
    fun addDateChangedListener(listener: (LocalDate?) -> Unit) {
        val token = inspectable.addEventHandler(
            ...
            Abi.IID_CalendarDatePickerDateChangedHandler,
            Abi.ICalendarDatePicker_add_DateChanged,
        ) { _, args ->
            val eventArgs = ComPtr(args)
            val boxed = eventArgs.getPtrOrNull(Abi.ICalendarDatePickerDateChangedEventArgs_get_NewDate)
    ...
    fun removeDateChangedListener(listener: (LocalDate?) -> Unit) {
        ...
        inspectable.removeEventHandler(Abi.ICalendarDatePicker_remove_DateChanged, token)
    }
}
