...
- ABI 値 (IID / vtable スロット / enum 値) は**必ず** `tools/dump_winmd.py` で winmd から抽出する。
  記憶・推測・手書きの値は禁止 (XamlInterop.kt 冒頭の方針)。C# ドキュメントの値もそのまま信用しない。
...
- IControl / IContentControl / IButtonBase / IUIElement / IFrameworkElement など
  **定義済みの共通インターフェースは再定義せず再利用する**。追加前に *Interop.kt を検索すること
...
- インスタンス生成 (手順 2 のクラス情報で分岐):
  - `composable factory` あり → `Activation.composeDefault(XamlInterop.CLS_X, XamlInterop.IID_IXFactory)`
    (戻り値は既定インターフェースのポインタ)
  - `activatable factory: <default IActivationFactory>` →
    `Activation.activate(XamlInterop.CLS_X).queryInterface(XamlInterop.IID_IX)`
- **com.appkitbox.winui4k / internal.winui パッケージで `java.lang.foreign` を import しない** (FFI バックエンド
